...
import { UploadIcon, ClearIcon, ClipboardIcon } from "./icons";
...
const EmailInput: React.FC<EmailInputProps> = ({
  ...
  const processFile = async (file: File | undefined) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setFileError(
        "Formato de arquivo inválido. Por favor, envie apenas .txt ou .pdf."
      );
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-gray-800/60 p-6 rounded-2xl shadow-xl backdrop-blur-lg border border-gray-700">
      ...
      <div className="mt-3 text-center sm:text-left">
        ...
        {!fileError && (
          <p className="text-xs text-gray-500">Formatos aceitos: .txt e .pdf</p>
        )}
      </div>
    </div>
  );
